var util = require('util'),
	...
	fs = require('fs'),
	JSON_Handler = require('./json.js').JSON_Handler;
...
var ON_STATEMENT = '__njsOnStatement__({statement_type: "%s", function_id: "%s", file_name: "%s", line: "%s", column: "%s", end_line: "%s", end_column: "%s"});';
...
function get_function_id(node, self, function_id, filename) {
	var current_node = node;
	while (current_node.parent !== undefined) {
		current_node = current_node.parent;
		var function_name = self.getFunctionName(current_node);
		if (function_name) {
			// /home/wolf/Ceges/McBaby/hello_lib.js@main:2:5
			return function_id = util.format("%s@%s:%s:%s", filename, function_name, current_node.loc.start.line-1, current_node.loc.start.column);
		}
	}
}

var McCabe_statements = {
	functions: [
		{
			function_id: "",
			statements: {}
...
// json_handler.obj.functions.push({function_id: "HELLO", statements: {}});
// json_handler.flush();
// console.log(json_handler.obj);
...
Injector.prototype.injectTracing = function(filename, code, wrapFunctions, includeArguments, wrappedFile) {
	...
	var output = esparse(code, {range: true, loc: true, ecmaVersion: 10}, function processASTNode(node) {
		...
		// Let the app know that there was an exception so it can adjust the stack trace or whatever
			...
			var function_id = get_function_id(node, self, name, filename);

			var statement = node.source();
			var on_statement = util.format(ON_STATEMENT, statement_type, function_id, filename, startLine, node.loc.start.column, retLine, node.loc.end.column);
