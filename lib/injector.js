...
var TRACE_EXIT = '__njsTraceExit__({entryData: __njsEntryData__, exception: __njsThrow__, line: %s, returnValue: %s});';
...
Injector.prototype.getFunctionName = function(node) {
	...
	// If we got no name ignore this function, this is interesting situation so ask users to report about it
...
Injector.prototype.injectTracing = function(filename, code, wrap) {
	...
	var output = falafel(code, {range: true, loc: true}, function(node) {
		...
		if (name) {
			...
			var funcBody = node.body.source();
			funcBody = funcBody.slice(1, funcBody.length - 1); // Remove the open and close braces "{}"
			...
			traceExit = util.format(TRACE_EXIT, node.loc.start.line, 'undefined');

			if (wrap) {
				node.update(funcDec + '{\ntry\n{\nvar __njsThrow__=\'false\';\n' + traceEntry + '\n' + funcBody + '\n' + traceExit + '\n} catch(__njsEX__) {\nif (__njsThrow__===\'false\') {\n__njsThrow__=\'TRUE\';\n' + traceExit + '\n}\n throw __njsEX__;\n}\n}');
			} else {
				node.update(funcDec + '{\nvar __njsThrow__=\'false\';\n' + traceEntry + '\n' + funcBody + '\n' + traceExit + '\n}');
			}
		...
		// If this is a return statement we should trace exit
			...
			if (node.argument) {
				...
				var tmpVar = '__njsTmp' + parseInt(Math.random()*10000) + '__';

				// We wrap the entire thing in a new block for cases when the return stmt is not in a block (i.e "if (x>0) return;").
				traceExit = util.format(TRACE_EXIT, node.loc.start.line, tmpVar);
		...
		// If this is a throw statement we trace it as exit
		} else if (node.type === Syntax.ThrowStatement) {
            traceExit = util.format(TRACE_EXIT, node.loc.start.line, 'undefined');
			// We set __njsThrow__ so in case we will wrap this whole function in try/catch we would know the exception was traced
            node.update('{\n__njsThrow__=\'true\';\n' + traceExit + '\n' + node.source() + '\n}');
		}
	});
