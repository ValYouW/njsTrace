...
/**
 * @typedef Injector~Config
 * @property {string} entryHandler The name of the global function to call on functions entry
 * @property {string} exitHandlerThe name of the global function to call on functions exit
 */
 ...
 * @param {Injector~Config} config - The injector config
 * @constructor
 */
function Injector(njsTrace, config) {
	if (!config || !config.entryHandler || !config.exitHandler) {
...
Injector.prototype.findFunctionExit = function(fnNode) {
	...
	this.traverse(fnNode, function (node, path) {
		var i, parent;
		if (node.type === esprima.Syntax.ReturnStatement || node.type === esprima.Syntax.ThrowStatement) {
			// It is possible that this return/throw statement belongs to some other inner function,
			...
			for (i = 0; i < path.length; ++i) {
				...
				if (this.isFunctionNode(parent)) {
					if (parent.range === fnNode.range) {
						...
						exits.push({ exception: node.type === esprima.Syntax.ThrowStatement, start: node.range[0]});
					}
					break;
				}
			}
...
Injector.prototype.injectTracing = function(filename, code) {
	...
	for (var i = functionList.length - 1; i >= 0; i -= 1) {
		...
		this.njs.log('      Modifying %s at %s::%s', func.name, filename, func.node.loc.start.line);

		var signature = this.config.entryHandler + '({file:' + JSON.stringify(filename) + ', ' +
