 ...
 */
Injector.prototype.injectTracing = function(filename, code, wrapFunctions, wrappedFile) {
	var self = this;
    var traceExit;
	var output = falafel(code, {range: true, loc: true}, function(node) {
		...
		if (name) {
			...
			// the module is being required, we just use a "module startup" string to hint in the output that this is the module startup code
			var args = wrappedFile && node.loc.start.line === 1 ? '"module startup"' : 'arguments';
