...
var ON_IF = '__njsOnIfStatement__({function_name: "%s", file_name: "%s", line: %s, column: %s, end_line: %s, end_column: %s});';
...
Injector.prototype.injectTracing = function(filename, code, wrapFunctions, includeArguments, wrappedFile) {
	...
	var output = esparse(code, {range: true, loc: true, ecmaVersion: 10}, function processASTNode(node) {
		...
		// Let the app know that there was an exception so it can adjust the stack trace or whatever
			...
			node.body.update('{\n' + ON_CATCH + '\n' + origCatch + '\n}');
		} else if (node.type === Syntax.IfStatement) { // on If statement
			...
			var ifBlock = node.source();

			var pos = ifBlock.indexOf("{") + 1;

			var on_if = util.format(ON_IF, function_name, filename, node.loc.start.line, node.loc.start.column, node.loc.end.line, node.loc.end.column);
			var newBlock = ifBlock.slice(0, pos) + "\n" + on_if + "\n" + ifBlock.slice(pos); // console.log ("IF STATEMENT")

			node.update(newBlock);
		} else if (node.type === Syntax.ForStatement){ // on For statement
			var current_node = node;
			var function_name;
			while (true) {
				current_node = current_node.parent;
			...
			var forBlock = node.source();

			var pos = forBlock.indexOf("{") + 1;

			var on_for = util.format(ON_FOR, function_name, filename, startLine, node.loc.start.column, retLine, node.loc.end.column);
			var newBlock = forBlock.slice(0, pos) + "\n" + on_for + "\n" + forBlock.slice(pos); // console.log ("IF STATEMENT")
