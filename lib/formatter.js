...
Formatter.prototype.onEntry = function(args) {
	...
	for (var i = 0; args.args && i < args.args.length && i < this.config.inspectArgsCount; ++i) {
		var currArg = util.inspect(args.args[i], this.config.inspectOptions);
		if (currArg.length > this.config.inspectArgsMaxLen) {
...
Formatter.prototype.onExit = function(args) {
	...
	var retVal = args.returnValue !== null ? util.inspect(args.returnValue, this.config.inspectOptions) : '';
	if (retVal.length > this.config.inspectArgsMaxLen) {
