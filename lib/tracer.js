...
var TRACE_OUT = '%s<-- %s, retLine: %s, retVal:%s';
var TRACE_OUT_EX = '%s<-- %s, EXCEPTION, retLine: %s, retVal:%s';
...
Tracer.prototype.onEntry = function(args) {
	...
	this.traceOut.write('%s--> %s, args:%s', this.spacing, stackFrame);
...
Tracer.prototype.popFrame = function(stackId) {
	...
	var res = this.stackMap[currStackId];
