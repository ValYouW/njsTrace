...
/**
 * Called when the code hits an "If" clause.
 * @param {NJSTrace.onIfStatement} args - The event args
 * @protected
 */
Tracer.prototype.onIfStatement = function(args) {
//						       S	   P      FN     L      C      EL    EC
	console.log(sprintf("%-12s | %-20s | %-40s | %-9s | %-6s | %-8s | %-9s |", 'IF CLAUSE',
		args.function_name, args.file_name, args.line, args.column, args.end_line, args.end_column));
	// console.log(args)
}

/**
 * Called when the code hits a "For" lop (for-in, for-each not included).
 * @param {NJSTrace.onForStatement} args - The event args
 * @protected
 */
Tracer.prototype.onForStatement = function(args) {
//						       S	   P      FN     L      C      EL    EC
	console.log(sprintf("%-12s | %-20s | %-40s | %-9s | %-6s | %-8s | %-9s |", 'FOR LOOP',
		args.function_name, args.file_name, args.line, args.column, args.end_line, args.end_column));
}
