...
Tracer.prototype.onEntry = function(args) {
	...
	for (var i = 0; i < this.formatters.length; ++i) {
		this.formatters[i].onEntry(args.name, args.file, args.line, this.stack);
	}
...
Tracer.prototype.onExit = function(args) {
	...
	for (var i = 0; i < this.formatters.length; ++i) {
		this.formatters[i].onExit(args.entryData.name, args.entryData.file, args.entryData.fnLine, args.line, this.stack, ts, args.exception);
	}
};
